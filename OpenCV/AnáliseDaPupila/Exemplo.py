...
loadImage = cv2.imread("olho.jpg")
cv2.imshow("Imagem", loadImage)

copia = loadImage.copy()

# Extrai as cores entre o intervalo BGR definido
mask = cv2.inRange(loadImage, (0, 0, 0), (60, 60, 60))
cv2.imshow("Imagem2", mask)


# slice no preto
imask = mask < 255

preto = np.zeros_like(loadImage, np.uint8)
preto[imask] = loadImage[imask]
...
if circles is not None:
    ...
    # Mostra a imagem com o círculo e centro encontrado pelo Hough Circle
    cv2.imshow("output", np.hstack([loadImage, copia]))
    cv2.waitKey(0)
...
matriz = np.array([[ -1,  -1,  -1],
                   [ -1, 8.33,  -1],
                   [-1, -1, -1]])
Saida = cv2.filter2D(loadImage, -1, matriz)
